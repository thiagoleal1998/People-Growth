...
const PROMPT_INSTRUCTIONS = `You are an editor for a Brazilian Portuguese business/growth news and consulting site. Read the article below and write two short editorial texts in Brazilian Portuguese, in plain prose with no markdown formatting (no "**", "##", bullet points, or quotes):

1. "excerpt_pt" — a one-sentence subtitle/dek (roughly 120-160 characters). It appears under the headline in article listings, cards, and as the search-engine description. It must NOT just restate the title in other words — it should add the specific angle, stake, or detail that makes someone want to read further.

2. "summary_pt" — a short paragraph (3-5 sentences) for a "Resumo" box shown at the top of the article, letting a reader who won't read the whole piece get the real substance. Prioritize by actual weight: the main argument or finding, the most concrete facts/data/names mentioned, and the conclusion or takeaway — not just the opening paragraph reworded. Skip minor color or examples that aren't load-bearing for the article's point.
