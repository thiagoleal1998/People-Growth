...
export default async function AdminDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: configData }, profile, { count: pendingComments }, { count: newLeads }, { count: newErrors }, { count: openTickets }] = await Promise.all([
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      <AdminSidebar
        ...
        userPhoto={userPhoto}
        counts={{ comentarios: pendingComments ?? 0, leads: newLeads ?? 0, chamados: (newErrors ?? 0) + (openTickets ?? 0) }}
      />
