  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "#64748b", fontSize: "0.9375rem" }}>Bem-vindo! Aqui está o resumo do seu site.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1.25rem", marginBottom: "2.5rem" }}>
        {stats.map(({ label, value, icon: Icon, color }) => (
          <div key={label} style={{ backgroundColor: "white", borderRadius: "1rem", padding: "1.5rem", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ fontWeight: 900, fontSize: "2rem", color: "#0d1b2a", lineHeight: 1, marginBottom: "0.25rem" }}>{value.toLocaleString("pt-BR")}</div>
      ...
      <div style={{ backgroundColor: "white", borderRadius: "1rem", border: "1px solid rgba(0,0,0,0.06)", overflow: "hidden" }}>
        ...
        {recentLeads.length === 0 ? (
          <div style={{ padding: "3rem", textAlign: "center", color: "#94a3b8", fontSize: "0.9rem" }}>Nenhum lead recebido ainda.</div>
        ) : (
                ...
                {recentLeads.map((lead) => {
                  ...
                  return (
                    <tr key={lead.id} style={{ borderTop: "1px solid #f1f5f9" }}>
                      <td style={{ padding: "1rem 1.5rem", fontWeight: 700, color: "#0d1b2a", fontSize: "0.9rem" }}>{lead.name}</td>
                      <td style={{ padding: "1rem 1.5rem", color: "#64748b", fontSize: "0.875rem" }}>{lead.email}</td>
                      <td style={{ padding: "1rem 1.5rem", color: "#475569", fontSize: "0.875rem" }}>{lead.service_interest ?? "—"}</td>
                      ...
                      <td style={{ padding: "1rem 1.5rem", color: "#94a3b8", fontSize: "0.8125rem" }}>{formatDate(lead.created_at)}</td>
                    </tr>
                  );
                })}
              </tbody>
