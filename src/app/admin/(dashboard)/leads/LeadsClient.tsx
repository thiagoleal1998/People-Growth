                  ...
                  return (
                    <Fragment key={lead.id}>
                      <tr style={{ borderTop: "1px solid #f1f5f9" }}>
                        <td style={{ padding: "0.875rem 1.25rem", fontWeight: 700, color: "#0d1b2a", fontSize: "0.875rem" }}>{lead.name}</td>
                        <td style={{ padding: "0.875rem 1.25rem", color: "#64748b", fontSize: "0.875rem" }}>{lead.email}</td>
                        <td style={{ padding: "0.875rem 1.25rem", color: "#64748b", fontSize: "0.875rem" }}>{lead.phone ?? "—"}</td>
                        <td style={{ padding: "0.875rem 1.25rem", color: "#475569", fontSize: "0.875rem" }}>{lead.service_interest ?? "—"}</td>
                        ...
                        <td style={{ padding: "0.875rem 1.25rem", color: "#94a3b8", fontSize: "0.8125rem", whiteSpace: "nowrap" }}>{formatDate(lead.created_at)}</td>
                        <td style={{ padding: "0.875rem 1.25rem" }}>
                          <button
                          ...
                          <td colSpan={8} style={{ padding: "1rem 1.25rem" }}>
                            {lead.message && (
                              <div style={{ marginBottom: "0.75rem" }}>
                                <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "#64748b", marginBottom: "0.25rem" }}>MENSAGEM</div>
                                <div style={{ fontSize: "0.875rem", color: "#334155" }}>{lead.message}</div>
                              </div>
                            )}
...
function NotesEditor({ leadId, initialNotes }: { leadId: string; initialNotes: string }) {
  ...
  return (
    <div>
      <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "#64748b", marginBottom: "0.25rem" }}>NOTAS INTERNAS</div>
      <textarea
        value={notes}
        onChange={(e) => { setNotes(e.target.value); setSaved(false); }}
        rows={2}
        style={{ width: "100%", padding: "0.5rem 0.75rem", borderRadius: "0.5rem", border: "1px solid #cbd5e1", fontSize: "0.875rem", boxSizing: "border-box", fontFamily: "inherit", resize: "vertical" }}
      />
      <button
